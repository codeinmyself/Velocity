...
import xmu.mystore.logmgt.yzh.annotation.ServiceLogger;
...
public class ServiceLog {
	...
	// Controller层切点
	@Pointcut("@annotation(xmu.mystore.logmgt.yzh.annotation.ServiceLogger)")
	...
	public Object homeControllerLog(ProceedingJoinPoint pjp) throws Throwable {
		...
		log.setResult(obj.getClass().getSimpleName());
	...
	public static String getControllerMethodOperatorObject(JoinPoint joinPoint) throws Exception {
		...
		for (Method method : methods) {
			if (method.getName().equals(methodName)) {
				Class[] clazzs = method.getParameterTypes();
				if (clazzs.length == arguments.length) {
					operatorObject = method.getAnnotation(ServiceLogger.class).operatorObeject();
					break;
				}
			}
		}
