package xmu.mystore.goodsmgt.lcf.service.impls;
...
import xmu.mystore.goodsmgt.lcf.exception.MyException;
import xmu.mystore.goodsmgt.lcf.mapper.GoodsMapper;
import xmu.mystore.goodsmgt.lcf.mapper.StandMapper;
...
public class GoodsServiceImpl implements GoodsService {
	...
	public boolean addGoods(Goods goods) {
		try {
			...
			if (goodsMapper.selectBySerialCode(goods) != null || goods.getSerial_code().equals("")) {
				int num = goodsMapper.selectByStoreId(goods);
				goods.setSerial_code(goods.getStore_id().toString() + String.format("%06d", num+1));
			}
