package xmu.mall.usercenter.pjx.controller;
...
import xmu.mall.usercenter.pjx.model.Show_OrderItem;
import xmu.mall.usercenter.pjx.model.User_collect_goods;
import xmu.mall.usercenter.pjx.model.Order_info;
...
public class UserController
{
	...
	private IGoodsService wyh_IGoodsService;
	...
	public String userCollection(Model model,HttpSession session)
	{
		...
		List<xmu.mystore.goodsmgt.wyh.model.Goods> good_list = new ArrayList<xmu.mystore.goodsmgt.wyh.model.Goods>();
		
		for(int i =0 ;i<ucg_list.size();++i){
			good_list.add(wyh_IGoodsService.getGoodsByGoodsId(ucg_list.get(i).getGoods_id()));
		}
			
		model.addAttribute("goods_list", good_list);
		
		return "pjx/collection";
	}
	...
	public String deleteAddress(
			...
			HttpSession session)
	{
		usersAddressService.deleteUserAddressBuAddressID(id);
