...
//                $("#name").text(item.name);
                str += "<tr>" +
                    ...
                    "<td>" + item.website + "</td>" +
                    "<td>" + item.status + "</td>" +
                    "<td><button type=\"button\"  class=\"btn btn-default btn-xs\" onclick=\"showModify("+item.id+")\" >修改</button><button type=\"button\" class=\"btn btn-default btn-xs\" onclick=\"deleteBrand("+item.id+")\">删除</button> </td>"
                    "</tr>";
                })
            tbody.innerHTML=str;
        },
...
function showModify(id) {
    ...
    $.ajax({
        ...
        success:function (data) {
            ...
            $("#price").attr("placeholder",data.realPrice);
            var category = document.getElementById("category");
            for(i=0;i<category.length;i++){
                if(category[i].value == data.categoryId)
                    category[i].selected = true;
            }
            var brand = document.getElementById("brand");
            for(i=0;i<brand.length;i++){
                if(brand[i].value == data.brandId)
                    brand[i].selected = true;
            }
            $("#stock_count").attr("placeholder",data.stockCount);
            $("#weight").attr("placeholder",data.weight);
            $("#package_number").attr("placeholder",data.preSaleNumber);
            var divide = document.getElementById("divide");
            for(i=0;i<divide.length;i++){
                if(divide[i].value == data.isDividable)
                    divide[i].selected = true;
            }
            $("#default_express").attr("placeholder",data.defaultExpress);
        },
        error:function () {
            alert("error");
        }
