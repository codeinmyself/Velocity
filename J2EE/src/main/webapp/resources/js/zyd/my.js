function sure(oid){
	var mode=oid;
	if(alertMsg("确定该用户已付款吗？", mode)==true) return true;
	else return false;
}
function check(){
   var text=document.getElementById("awb").value;
   var mode=0;
   if(text==""){
	 alertMsg("快递单号不能为空！", mode) 
     return false;
   }
   return true;
}
function search(){
	...
	var mode=0;
	if(!num.test(orderId)){
		alertMsg("订单号必须是数字！", mode) 
		return false;
	}
	if(!num.test(goodsId)){
		alertMsg("商品ID必须是数字！", mode) 
		return false;
	}
	if(!dou.test(orderTotal)){
		alertMsg("总价必须是数字！", mode) 
		return false;
	}
	if(!num.test(userId)){
		alertMsg("下单人账号必须是数字！", mode) 
		return false;
	}
	return true;
}
function alertMsg(msg, mode) { //mode为空，即只有一个确认按钮，mode为1时有确认和取消两个按钮
    ...
    if (mode != 0) btn1.href = 'sureOrder?orderId='+mode;
    else btn1.href='javas' + 'cript:void(0)';
